...
<div class="press_line">
		...
		<?
			...
			foreach(array_reverse($arItem["PROPERTIES"]["FOTOS"]["VALUE"]) as $foto) {
					...
					$img2 = CFile::GetPath($foto);
					
					if ($i==0) { $imgArr[] = $img2;}
					
					$data_img .= "data-img".$j."=\"".$img2."\" ";
			...
			?>
								<div class="swiper-slide" data-hash="<?=$year?>">
									<div class="press_item">
										<a href="<?=$arItem["DETAIL_PAGE_URL"]?>" <?=$data_img?> data-img="<?=$img?>"  class="<?=$i==0 ? "selected" : ""?>" data-link="<?=$link?>">
...
<div class="width_wrapper" style="display:none">
		<div class="press_detail">
			...
			<img data-i="0" src="<?=$first_img?>">
			
			<?for($j=1;$j<11;$j++) {
				if ($imgArr[$j-1]) {
				?><img data-i="<?=$j?>" src="<?=$imgArr[$j-1]?>">
				<?} else {?>
				<img data-i="<?=$j?>" style="display:none;" src="">
